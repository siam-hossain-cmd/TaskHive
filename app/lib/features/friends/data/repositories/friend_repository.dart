...
class FriendRepository {
  ...
  Future<List<UserProfile>> searchUsers(String query) async {
    ...
    // 1️⃣ Try exact User-ID lookup (by auth UID or uniqueId)
    if (!q.contains('@') && q.length >= 6) {
      try {
        // First try by uniqueId (e.g., TH-A7X9K2)
        final uniqueIdSnap = await _usersRef
            .where('uniqueId', isEqualTo: q.toUpperCase())
            .limit(1)
            .get();
        
        if (uniqueIdSnap.docs.isNotEmpty) {
          final doc = uniqueIdSnap.docs.first;
          results.add(UserProfile.fromMap({
            ...doc.data() as Map<String, dynamic>,
            'uid': doc.id,
          }));
        } else {
          // Fallback to Auth UID lookup
  ...
  Stream<List<FriendRequestModel>> getIncomingRequests(String uid) {
    return _requestsRef
        ...
        .snapshots()
        .map((s) {
          final list = s.docs.map((d) => FriendRequestModel.fromFirestore(d)).toList();
